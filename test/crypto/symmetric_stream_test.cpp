...
TEST(CryptoTest, CanEncryptAndDecryptStreamUsingSymmetricEncryption) {
    auto encryption_key = symmetric_stream::makeEncryptionKey();
    auto encryption_stream = symmetric_stream::makeEncryptionStream(encryption_key);
    ...
    c2 = encryption_stream->write(m2, symmetric_stream_tag::_rekey);
    ...
    EXPECT_EQ(decryption_stream->lastTag(), symmetric_stream_tag::_message);
    EXPECT_TRUE(decryption_stream->isStreamOpen());

    std::string p2 = decryption_stream->write(c2);
    EXPECT_EQ(decryption_stream->lastTag(), symmetric_stream_tag::_rekey);
    EXPECT_TRUE(decryption_stream->isStreamOpen());
    ...
    EXPECT_EQ(decryption_stream->lastTag(), symmetric_stream_tag::_stream_end);
